...
#ifdef LOTTERY
	e->env_tickets = ENV_INITIAL_TICKETS;
#endif
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	total_tickets += ENV_INITIAL_TICKETS;
#endif
}
...
env_run(struct Env *e)
{
	...
	// Your code here
	if (curenv && curenv != e) {
		if (curenv->env_status ==
		    ENV_RUNNING)  // If current enviroment was running
			curenv->env_status =
			        ENV_RUNNABLE;  // Set status to runnable, not running anymore
	}

	curenv = e;  // Set current environment to the new one
	curenv->env_status = ENV_RUNNING;
	curenv->env_runs++;

	env_load_pgdir(curenv);  // Switch address space
