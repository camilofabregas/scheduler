...
#ifdef LOTTERY
static int
sys_env_upgrade_priority(envid_t envid)
{
	struct Env *env;
	...
	env->env_tickets = ENV_INITIAL_TICKETS;

	return 0;
}

static int
sys_env_downgrade_priority(envid_t envid)
{
	struct Env *env;
	int result = envid2env(envid, &env, 1);
	if (result != 0)
...
#ifdef LOTTERY
	// Inherit tickets from father process
	newenv->env_tickets = curenv->env_tickets;
	// Update total count of tickets
	total_tickets += newenv->env_tickets;
#endif
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_yield:
		sys_yield();  // No return
#ifdef LOTTERY
	case SYS_env_upgrade_priority:
		return sys_env_upgrade_priority(a1);
	case SYS_env_downgrade_priority:
		return sys_env_downgrade_priority(a1);
